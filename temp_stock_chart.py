...
<head>
<meta charset="UTF-8">
<title>{default_symbol} ΓÇö OpenFinCh</title>
<style>
...
<div id="header">
  ...
  <select id="interval-select">
    {interval_options_html}
    <option value="custom">CustomΓÇª</option>
  </select>
...
<div id="indicators-panel">
  <select id="indicator-select">
    <option value="" selected disabled>+ Add indicatorΓÇª</option>
...
async function fetchSymbol(symbol) {{
  ...
  try {{
    ...
    document.title = currentSymbol + ' ΓÇö OpenFinCh';
...
async function applyCustomInterval() {{
  ...
  if (!value || value < 1) {{ showToast('Enter a value ΓëÑ 1'); return; }}
...
chart.subscribeCrosshairMove(param => {{
  ...
  if (chartMode === 'candles' && candle) {{
    ...
    if (pt) {{
      lo.textContent = 'ΓÇö'; lo.className = 'val';
      lh.textContent = 'ΓÇö'; lh.className = 'val';
      ll.textContent = 'ΓÇö'; ll.className = 'val';
...
function detectLocalIndex() {{
  ...
  // Timezone ΓåÆ [primary index, secondary index] (pick first; secondary is fallback context)
  const tzMap = {{
    // India
    'Asia/Kolkata':      '^NSEI',
    'Asia/Calcutta':     '^NSEI',
